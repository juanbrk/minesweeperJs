...
class GameSummary extends Component {
    render() {
        ...
        const prevGameAsMap = this.props.gameData ? Object.entries(this.props.gameData) : null;

        // filter prevGameState  to display only   difficulty,  mineCOunt, movesCount && starttime 
        //filteredresults = [[key, value],[key, value]...]
        const filteredResults = prevGameAsMap ? displayTexts.map(text => {
        ...
        if (filteredResults) {
            ...
            gameSummary = filteredResults.map((itemArray) => {
                //itemArray === [[key, value]]
                let displayText = "";
